...
namespace {
	...
	namespace math {
		...
		struct Expression {
			using Polynomial = pmr::unordered_map<Expression*, ValueType>;
			...
			Expression(
				ValueType cof, const ExpPtr& expr
			) : marked{ false }, polynomial{} {
				polynomial[expr.get()] = cof;
			}
			Expression(
				ValueType lcof, const ExpPtr& lhs,
				ValueType rcof, const ExpPtr& rhs
			) : marked{ false }, polynomial{} {
				polynomial[lhs.get()] = lcof;
				polynomial[rhs.get()] = rcof;
			}
			...
			void addTerm(Expression* expr, ValueType val) {
			...
			void addExpresison(ValueType coef, Expression& other) {
				if (other.marked) {
					addTerm(&other, coef);
				}
				else {
...
void hiho::ad22_expr_umap_shrd_mrk_pmr(double s, double sigma, double k, double r, double t, int simulation)
{
