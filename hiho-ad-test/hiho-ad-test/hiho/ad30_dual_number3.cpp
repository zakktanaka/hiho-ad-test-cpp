...
namespace {
	...
	using Differential  = std::pair<size_t, ValueType>;
	using Differentials = pmr::vector<Differential>;
	...
	namespace math {
		...
		struct Number : public INumber {
			...
			void mark() { id_ = ++indexer; differentials_.emplace_back(id_, 1); }

			ValueType v() const override { return v_; }
			void update(Differentials& updated, ValueType coef) const override {
				for (auto& p : differentials_) {
					auto updatedflag = false;
					for (auto& up : updated) {
						if (up.first == p.first) {
							up.second += coef * p.second;
							updatedflag = true;
							break;
						}
					}
					if (!updatedflag) { updated.emplace_back(p.first, coef * p.second); }
				}
			}
			...
			ValueType d(const Number& x) const {
				for (auto& p : differentials_) {
					if(p.first == x.id_) { return p.second; }
				}
...
void hiho::ad30_dual_number3(double s, double sigma, double k, double r, double t, int simulation)
{
